...
El sistema genera 4 archivos CSV. Cada uno tiene un rol distinto y se genera por un script específico:
...
| `ENDURANCE_HRV_master_FINAL.csv` | El gate de entrenamiento, las sombras, el residual, y toda la auditoría necesaria para entender por qué el sistema tomó esa decisión. | `endurance_v4lite.py` | 49 |
| `ENDURANCE_HRV_master_DASHBOARD.csv` | Lo esencial para decidir en 10 segundos: semáforo, acción, y warning. Subconjunto de FINAL. | `endurance_v4lite.py` | 9 |
...
Estas reglas aplican a los 4 archivos por igual:
...
```
Fecha,Calidad,HR_today,RMSSD_stable,gate_badge,Action,gate_razon_base60,decision_path,baseline60_degraded
```
...
## 6. BETA_AUDIT (legacy) — columnas y orden exacto
...
```
              ...
              (medición)  (modelo beta V3)
                   ...
                   ▼
           ┌──────────────────┐
           │ endurance_v4lite │  Evalúa CORE → gate + acción
           └──────────────────┘
                   │
           ┌───────┴───────┐
           ▼               ▼
...
## 9. QA mínimo obligatorio (anti-bugs)
...
## 10. Resultado práctico (cómo se usa cada día)
