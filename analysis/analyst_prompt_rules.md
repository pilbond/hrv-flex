...
## Reglas de error_context y exit_context (SYA-11)
- si `session_payload.json.narrative_targets.error_context` existe, usarlo como ancla estructurada para `Donde Estuvo el Error`; no lo sustituyas por un texto generico
- `error_context.gate_mode` + `error_context.gate_vs_execution_delta` + `error_context.execution_coherence` determinan el tipo de error: `gate_first` + `exceeded` + `high` → error de decision, no de ejecucion; `gate_first` + `aligned` → el error, si lo hubo, fue de dosificacion interna; `caution_first` + `negative_cost_count > positive_count` → error de encaje en el bloque
- `error_context.cost_vs_gate_mismatch = true` debe nombrarse explicitamente cuando sea el caso: el ejecutor fue tecnicamente correcto pero la decision de nivel excedio el margen autorizado
- si `error_context.execution_coherence` no esta disponible (campo ausente), no inferas coherencia; deja esa dimension como no determinable
- si `error_context.thermal_penalty` o `error_context.durability_hint` estan presentes, incorporalos como agravantes del coste real, no como causas del error
- si `session_payload.json.narrative_targets.exit_context` existe, usarlo como ancla para `Como Habria Encajado Mejor`, `Que Construye vs Que Consume` y `Que Repetir / Que No Repetir`
- `exit_context.block_role_signals.is_peak_load_in_block` + `exit_context.block_role_signals.sessions_since_last_quality` permiten nombrar el rol real de la sesion en el bloque; usa ambos juntos para construir la frase de rol (p.ej. "pico de carga del bloque, primera sesion de calidad en ese deporte desde hace N sesiones")
- `exit_context.block_role_signals.load_rank_in_sport_7d = 1` significa que esta fue la sesion mas costosa del mismo deporte en los ultimos 7 dias; usalo para acotar si la dosis fue extrema o normal dentro del bloque reciente
- `exit_context.adaptation_signals.climb_count` + `exit_context.adaptation_signals.long_duration` + `exit_context.adaptation_signals.thermal_load` definen el contexto de estimulo para evaluar si la adaptacion es probable o si el coste real redujo el margen; jerarquiza las senales en tension, no las listes sin orden
- `exit_context.execution_quality.cost_within_expected = false` indica que el coste fue mayor de lo esperable para esa dosis (drift >= 10% o durability_hint fade_like sin gate restrictivo); usalo para matizar si "Como Habria Encajado Mejor" pasa por menos dosis, mas margen temporal o condiciones diferentes
- cuando `error_context` y `exit_context` esten disponibles, las secciones de juicio del bloque coaching deben derivar de esas anclas, no de inferencias genericas sobre el deporte o el gate en abstracto
- convencion de nulos en `error_context` y `exit_context`: los campos opcionales (`execution_coherence`, `thermal_penalty`, `durability_hint`, `climb_count`, `thermal_load`) se omiten del dict cuando no hay dato; si un campo no aparece, tratarlo como "no determinable" y no inferas su valor ni uses un fallback generico; los campos estructurales (`gate_mode`, `gate_vs_execution_delta`, `cost_vs_gate_mismatch`, `target_hit`, `cost_within_expected`) siempre estan presentes
- politica de solapamiento con `composite_context`: `thermal_penalty` en `error_context` y `thermal_load` en `exit_context.adaptation_signals` son proyecciones curadas de `composite_context.thermal_context.thermal_band`; `durability_hint` en ambos contextos es proyeccion de `composite_context.durability_context.durability_hint`; los valores son identicos porque vienen de la misma fuente; usa las anclas de `error_context`/`exit_context` como fuente primaria para las secciones de juicio y no las reconcilies con `composite_context` — si hubiera discrepancia (no deberia ocurrir) manda la ancla especifica
- degradacion con `rr_unavailable=true`: `error_context` y `exit_context` se construyen desde `session_row` y señales de `composite_context`, sin depender de RR; siguen siendo validos y usables cuando el analisis RR no esta disponible; si los campos opcionales de contexto termico o durabilidad estan ausentes, puede deberse a que tampoco habia stream CSV (no a que RR falle) — leerlo como "no determinable", no como señal positiva
- `exit_context.block_role_signals.load_rank_in_sport_7d` usa la columna `load` de `sessions.csv` (equivalente a TRIMP-HR, misma escala que aparece en la tabla de `Encaje en el bloque`); es coherente con el resto del pipeline y no equivale a `load_cost` ni a `training_load` de Intervals.icu
- si `error_context.version` o `exit_context.version` no coincide con la version que conoces, tratar el contexto como valido pero no asumir que todos los campos del esquema actual estan disponibles
