...
def _build_sessions_day_lookups(
...
) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame], Dict[str, float]]:
    ...
    if not present_ctx_cols:
        return exact_lookup, None, {}
    ...
    return exact_lookup, ctx_lookup, thresholds
...
def build_final_and_dashboard(core: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if sday_path.exists():
        try:
            ...
            sday_lookup, load_ctx_lookup, load_ctx_thresholds = _build_sessions_day_lookups(sday_df)
        except (OSError, ValueError, KeyError):
            sday_lookup = None
