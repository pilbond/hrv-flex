...
| `data/audit/polar_v4_shadow.jsonl` | Sidecar de auditoría Polar v4 (AYO-19): append-only JSONL con un registro por fecha. Presencia/diffs/latencia/refreshes de v4 frente a v3 (sleep.csv, CORE.csv). Se genera solo bajo `POLAR_API_VERSION=shadow` en el final de cada `--process`. No modifica ningún contrato canónico ni interfiere con el pipeline principal. Formato: `{date, captured_at_utc, domains: {sleep: {...}, nightly: {...}, sessions: {...}}}`. | `hrv_app.polar_shadow` (durante `--process` si `POLAR_API_VERSION=shadow`) | — |
