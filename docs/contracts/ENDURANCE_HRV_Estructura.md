...
| `ENDURANCE_HRV_sleep.csv` | Sueño nocturno y señales de recuperación (Polar). Alimenta el reason_text pero NO afecta al gate. | `sleep_store.py` (coordinado por `polar_hrv_automation.py`) | 17 |
| `ENDURANCE_HRV_sessions.csv` | Detalle de cada sesión de entrenamiento: zonas, work blocks, drift, effort, clasificación y capa mecánica opcional. | `build_sessions.py` | 58 |
...
```
  ...
  Polar Nightly  ───┤
                    ...
                    │
  Intervals.icu ────┤
├──► build_sessions.py ──► SESSIONS.csv (58 cols)
