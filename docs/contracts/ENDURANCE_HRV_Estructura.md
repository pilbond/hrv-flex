...
| `ENDURANCE_HRV_ssm_shadow.csv` | Sidecar técnico sombra con estado latente HRV+carga, observación nocturna auxiliar, incertidumbre, flags de calidad y controles simples (`control_rolling_hrv_7d`, `control_load_7d`). No toca `FINAL`, no recolorea el gate y no se expone en la UI operativa. | `build_hrv_ssm.py` | 30 |
