...
Como fallback secundario, el pipeline puede enriquecer con muestras mecánicas de Polar AccessLink (v4): `GET /v4/data/training-sessions/list` + catálogo `/v4/data/sports/list`.
...
**Cobertura Polar:** cuando `mechanics_source = polar`, la cobertura depende de la ventana reciente expuesta por Polar AccessLink v4: `/v4/data/training-sessions/list` con `features=samples`, catálogo desde `/v4/data/sports/list`, scopes `training_sessions:read` y `sports:read`. No debe asumirse como fuente de backfill histórico completo.
