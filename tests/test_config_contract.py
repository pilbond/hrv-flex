    ...
    def test_path_contract_uses_data_dir_and_rr_download_dir(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(
                ...
                self.assertEqual(config.SESSIONS_DAY_PATH, data_dir / "ENDURANCE_HRV_sessions_day.csv")
                self.assertEqual(config.FINAL_PATH, data_dir / "ENDURANCE_HRV_master_FINAL.csv")
                self.assertEqual(config.FINAL_REASON_ITEMS_PATH, data_dir / "ENDURANCE_HRV_master_FINAL_reason_items.json")
